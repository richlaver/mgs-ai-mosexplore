...
class TimeSeriesPlotSandboxAgentInput(BaseModel):
    prompt: str = Field(description="""
Natural language description of the time series plot including:
...
def create_timeseries_plot_sandbox_subgraph(llm: BaseLanguageModel, sql_tool: GeneralSQLQueryTool, write_artefact_tool: WriteArtefactTool, thread_id: str, user_id: int) -> CompiledStateGraph:
    ...
    You are an expert at formulating inputs for the TimeSeriesPlotTool.

    Tool input schema (must match exactly; output as JSON):
    - primary_y_instruments: List of dicts with 'instrument_id' (str, existing in DB) and 'column_name' (str, 'dataN' or 'calculationN').
    - secondary_y_instruments: Optional list of similar dicts.
    - start_time: Str in 'D Month YYYY H:MM:SS AM/PM' (e.g., '1 January 2025 12:00:00 PM').
    - end_time: Similar str, later than start.
    - primary_y_title: Str describing primary data.
    - primary_y_unit: Str unit.
    - secondary_y_title: Optional str, required if secondary instruments.
    - secondary_y_unit: Optional str, required if secondary instruments.
    - review_level_values: Optional list of floats (max 3 pos, 3 neg), not with secondary.
    - highlight_zero: Optional bool.
    ...
    Task:
    1. Parse prompt for instruments, times, titles, units, etc.
    2. Ensure times match format; start < end.
    3. Validate constraints (e.g., total instruments <=7).
    ...
    7. If start time not specified, use 1 January 1900 12:00:00 AM.
    """)
    ...
    def generate_inputs(state: TimeSeriesPlotSandboxAgentState) -> TimeSeriesPlotSandboxAgentState:
        logger.debug("Entering generate_inputs with state: %s", json.dumps(state, default=str))
        t0 = time.perf_counter()
        messages = state.get('messages', [])
        messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Generating tool inputs...", additional_kwargs={"stage": "intermediate", "process": "action"}))
        ...
        try:
            ...
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content=f"Generated inputs: {json.dumps(tool_inputs)}", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            ...
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content=error_msg, additional_kwargs={"stage": "intermediate", "process": "observation"}))
    ...
    def call_tool(state: TimeSeriesPlotSandboxAgentState) -> TimeSeriesPlotSandboxAgentState:
        logger.debug("Entering call_tool with state: %s", json.dumps(state, default=str))
        messages = state.get('messages', [])
        messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Calling TimeSeriesPlotTool...", additional_kwargs={"stage": "intermediate", "process": "action"}))
        ...
        try:
            ...
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Tool called successfully.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
    ...
    def decide_next_node(state: TimeSeriesPlotSandboxAgentState) -> TimeSeriesPlotSandboxAgentState:
        ...
        if should_retry and new_attempt < 5:
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content=f"Retrying due to: {tool_error[:100]}... Attempt {new_attempt}/5.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            ...
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Proceeding to parse result." if tool_result else "Max attempts reached; returning None.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            logger.info("Deciding to proceed: next_path=parse_result")
            return {"next_path": "parse_result", "messages": messages}

    def parse_result(state: TimeSeriesPlotSandboxAgentState) -> TimeSeriesPlotSandboxAgentState:
        logger.debug("Entering parse_result with state: %s", json.dumps(state, default=str))
        messages = state.get('messages', [])
        messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Parsing final result...", additional_kwargs={"stage": "intermediate", "process": "action"}))
        tool_result = state.get('tool_result')
        if not tool_result or not tool_result.get('artefacts'):
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content="No plot generated.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            logger.warning("parse_result: No plot generated")
        ...
        for a in tool_result['artefacts']:
            ...
            try:
                output = write_artefact_tool._run(
                    ...
                    thread_id=thread_id,
                    user_id=user_id,
                    generating_tool='TimeSeriesPlotTool',
                    generating_parameters=generating_parameters,
                    description='Plotly JSON for time series plot'
                )
                if output['error']:
                ...
                return {"tool_result": None, "messages": messages}
        if artefact_id:
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content="Plot artefact stored successfully.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            logger.debug("parse_result: Plot generated and stored successfully")
            return {"tool_result": artefact_id, "messages": messages}
        else:
            messages.append(AIMessage(name="TimeSeriesPlotAgent", content="No Plotly JSON artefact stored.", additional_kwargs={"stage": "intermediate", "process": "observation"}))
            logger.warning("parse_result: No Plotly JSON artefact stored")
            return {"tool_result": None, "messages": messages}

    graph = StateGraph(TimeSeriesPlotSandboxAgentState)
...
class TimeSeriesPlotSandboxAgentTool(BaseTool):
    ...
    def _run(self, prompt: str) -> Optional[str]:
        logger.debug("Entering TimeSeriesPlotSandboxAgentTool._run with prompt: %s", prompt)
        graph = create_timeseries_plot_sandbox_subgraph(self.llm, self.sql_tool, self.write_artefact_tool, self.thread_id, self.user_id)
        initial_state = {
            "prompt": prompt,
            "attempt_count": 0,
            "messages": [AIMessage(name="TimeSeriesPlotAgent", content="Starting plot generation.", additional_kwargs={"stage": "intermediate", "process": "action"})]
        }
...
def timeseries_plot_sandbox_agent(llm: BaseLanguageModel, sql_tool: GeneralSQLQueryTool, write_artefact_tool: WriteArtefactTool, thread_id: str, user_id: int) -> BaseTool:
    logger.debug("Creating TimeSeriesPlotSandboxAgentTool instance")
    return TimeSeriesPlotSandboxAgentTool(llm=llm, sql_tool=sql_tool, write_artefact_tool=write_artefact_tool, thread_id=thread_id, user_id=user_id)